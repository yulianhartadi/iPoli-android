...
class MembershipViewController(args: Bundle? = null) :
    ...
    override fun render(state: MembershipViewState, view: View) {
        ...
        when (state.type) {
            DATA_CHANGED -> {
                ...
                view.monthlyPlanContainer.dispatchOnClick(MembershipAction.SelectPlan(mypoli.android.store.membership.MembershipPlan.MONTHLY))
                view.yearlyPlanContainer.dispatchOnClick(MembershipAction.SelectPlan(mypoli.android.store.membership.MembershipPlan.YEARLY))
                view.quarterlyPlanContainer.dispatchOnClick(MembershipAction.SelectPlan(mypoli.android.store.membership.MembershipPlan.QUARTERLY))
    ...
    private val MembershipViewState.showMostPopular: Boolean
        get() = selectedPlan == mypoli.android.store.membership.MembershipPlan.YEARLY
