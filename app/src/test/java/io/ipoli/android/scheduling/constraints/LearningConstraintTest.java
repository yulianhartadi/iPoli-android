...
public class LearningConstraintTest {
    ...
    public void shouldHaveEqualProbabilitiesInFirstPeakEdges() {
        DailySchedule schedule = new DailySchedule(0, 15 * 60, 15);
        DiscreteDistribution dist = constraint.apply(schedule);
        assertThat(dist.at(schedule.getSlotForMinute(2 * 60)),
                is(dist.at(schedule.getSlotForMinute(4 * 60))));
    }
    ...
    public void shouldHaveHigherProbabilityInPeakThanSlope() {
        DailySchedule schedule = new DailySchedule(0, 15 * 60, 15);
        DiscreteDistribution dist = constraint.apply(schedule);
        assertThat(dist.at(schedule.getSlotForMinute(3 * 60)),
                is(greaterThan(dist.at(schedule.getSlotForMinute(4 * 60 + 15)))));
    }
    ...
    public void shouldHaveHigherProbabilityInSlopeThanPlateau() {
        DailySchedule schedule = new DailySchedule(0, 15 * 60, 15);
        DiscreteDistribution dist = constraint.apply(schedule);
        assertThat(dist.at(schedule.getSlotForMinute(4 * 60 + 15)),
                is(greaterThan(dist.at(schedule.getSlotForMinute(6 * 60)))));
    }
}
