...
public class UpgradeStatusCheckerTest {
    ...
    public void checkStatus_PlayerWithoutMembershipAndValidUpgrades_EmptyLists() {
        long createdAtExpiredTrial = DateUtils.toMillis(LocalDate.now().minusDays(Constants.UPGRADE_TRIAL_PERIOD_DAYS + Constants.UPGRADE_GRACE_PERIOD_DAYS + 1));
        player.setCreatedAt(createdAtExpiredTrial);
        player.getInventory().addUpgrade(Upgrade.CHALLENGES, LocalDate.now().plusDays(Constants.UPGRADE_GRACE_PERIOD_DAYS + 1));
        UpgradeStatusChecker checker = new UpgradeStatusChecker(player, LocalDate.now());
        UpgradeStatus status = checker.checkStatus();
        assertThat(status.inGracePeriod.size(), is(0));
        assertThat(status.expired.size(), is(0));
        assertThat(status.toBeRenewed.size(), is(0));
        assertThat(status.type, is(UpgradeStatus.StatusType.NOT_MEMBER));
    }
    ...
    public void checkStatus_PlayerInTrial_TrialStatus() {
        ...
        UpgradeStatusChecker checker = new UpgradeStatusChecker(player, LocalDate.now());
        UpgradeStatus status = checker.checkStatus();
        assertThat(status.type, is(UpgradeStatus.StatusType.TRIAL));
    }
    ...
    public void checkStatus_PlayerInTrialGrace_TrialGraceStatus() {
        ...
        UpgradeStatusChecker checker = new UpgradeStatusChecker(player, LocalDate.now());
        UpgradeStatus status = checker.checkStatus();
        assertThat(status.type, is(UpgradeStatus.StatusType.TRIAL_GRACE));
